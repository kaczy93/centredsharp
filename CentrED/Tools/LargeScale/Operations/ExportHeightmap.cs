...
public class ExportHeightmap : LocalLargeScaleTool
{
    ...
    private Image<L8>? _exportFileGrayscale;
    private Image<Rgb24>? _exportFileColored;
    ...
    protected override bool DrawToolUI()
    {
        ...
        if (ImGui.Button("..."))
        {
            string[] filters = _coloredMode ? ["*.png", "*.bmp"] : ["*.bmp"];
            if (TinyFileDialogs.TrySaveFile
                    (LangManager.Get(SELECT_FILE), Environment.CurrentDirectory, filters, null, out var newPath))
            {
        ...
        changed |= ImGui.Checkbox(LangManager.Get(EXPORT_HEIGHTMAP_COLORED), ref _coloredMode);
        ImGuiEx.Tooltip(LangManager.Get(EXPORT_HEIGHTMAP_COLORED_TOOLTIP));
    ...
    {
        var validExtensions = _coloredMode ? new[] { ".bmp", ".png" } : new[] { ".bmp" };
        if (string.IsNullOrEmpty(_exportFilePath) || !validExtensions.Any(_exportFilePath.EndsWith))
        {
            _submitStatus = _coloredMode
                ? LangManager.Get(INVALID_FILE_NOT_BMP_PNG)
                : LangManager.Get(INVALID_FILE_NOT_BMP);
            return false;
        }
    ...
    protected override void PreProcessArea(CentrEDClient client, RectU16 area)
    {
        ...
        if (_coloredMode)
            _exportFileColored = new Image<Rgb24>(area.Width, area.Height);
        else
            _exportFileGrayscale = new Image<L8>(area.Width, area.Height);
        xOffset = area.X1;
        yOffset = area.Y1;
    }
    ...
    protected override void ProcessTile(CentrEDClient client, ushort x, ushort y)
    {
        ...
        if (_coloredMode)
        {
            var color = GetBiomeColor(landTile.Id, z);
            _exportFileColored![x - xOffset, y - yOffset] = color;
        }
        else
        {
            var value = (byte)(z + 128);
            _exportFileGrayscale![x - xOffset, y - yOffset] = new L8(value);
        }
    }
    ...
    protected override void PostProcessArea(CentrEDClient client, RectU16 area)
    {
        ...
        if (_coloredMode)
        {
            if (_exportFilePath.EndsWith(".png"))
                _exportFileColored!.Save(fileStream, new PngEncoder());
            else
                _exportFileColored!.Save(fileStream, new BmpEncoder { BitsPerPixel = BmpBitsPerPixel.Pixel24 });
            _exportFileColored.Dispose();
            _exportFileColored = null;
        }
        else
        {
            _exportFileGrayscale!.Save(fileStream, new BmpEncoder { BitsPerPixel = BmpBitsPerPixel.Pixel8 });
            _exportFileGrayscale.Dispose();
            _exportFileGrayscale = null;
        }
    }

    private Rgb24 GetBiomeColor(ushort tileId, sbyte z)
    {
