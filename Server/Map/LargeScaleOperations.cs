...
public abstract class LargeScaleOperation
{
    ...
    public abstract void Apply
        (LandTile landTile, IEnumerable<StaticTile> staticTiles, ref bool[] additionalAffectedBlocks);
        ...
        (LandTile landTile, IEnumerable<StaticTile> staticTiles, ref bool[] additionalAffectedBlocks)
    {
        ...
        switch (_type)
        {
            case CopyMove.Copy:
            {
                ...
                {
                    targetStaticsBlock.AddTile(new StaticTile(staticTile.Id, x, y, staticTile.Z, staticTile.Hue));
                }
                break;
            }
            case CopyMove.Move:
...
public class LsDeleteStatics : LargeScaleOperation
{
        ...
        (LandTile landTile, IEnumerable<StaticTile> staticTiles, ref bool[] additionalAffectedBlocks)
    {
        ...
        foreach (var staticTile in staticTiles)
        {
            if (staticTile.Z < _minZ || staticTile.Z > _maxZ)
                continue;
...
public class LsInsertStatics : LargeScaleOperation
{
    ...
    public override void Apply
        (LandTile landTile, IEnumerable<StaticTile> staticTiles, ref bool[] additionalAffectedBlocks)
    {
        if (_tileIds.Length == 0 || random.Next(100) >= _probability)
            return;

        var staticItem = new StaticTile(_tileIds[random.Next(_tileIds.Length)], landTile.X, landTile.Y, 0, 0);
        switch (_placementType)
        {
            ...
            case StaticsPlacement.Top:
            {
                ...
                foreach (var staticTile in staticTiles)
                {
                    ...
                    (
                        (sbyte)(staticTile.Z + _landscape.TileDataProvider.StaticTiles[staticTile.Id].Height),
                        (sbyte)-128,
                        (sbyte)127
                    );
                    if (staticTop > topZ)
